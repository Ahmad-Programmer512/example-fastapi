...
def test_vote_on_post_non_exists(authorized_client, test_posts):
    res = authorized_client.post("/votes/", json={"post_id": 212121, "dir": 1})

    assert res.status_code == 404

def test_unauthenticated_vote_post(client, test_posts):
    res = client.post("/votes/", json={"post_id": 890890, "dir": 0})
